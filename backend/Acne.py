...
# ---------- 基本設定 ----------
PROC = psutil.Process(os.getpid())   # 計時
tasks = {}          # {task_id: progress (0~100)}
...
def classify_acne(img_path):
    """回傳分析結果：每類痘痘的數量與信心值"""
    if model is None:
        return 'AI model not loaded', 'N/A', 0 , {}

    t0 = time.perf_counter()         # ── 計時起點
    mem0 = PROC.memory_info().rss    # ── 起始 RSS (bytes)
    ...
    try:
        ...
        return f'Error during YOLO classification: {e}', 'N/A', 0 , {}
    
    finally: # 計時處理時間
        dt_ms  = (time.perf_counter() - t0) * 1_000     # 毫秒
        mem_mb = (PROC.memory_info().rss - mem0) / 1_048_576
        print(f"[Profile] Inference {dt_ms:.1f} ms, +{mem_mb:.2f} MB")
...
def upload():
    ...
    for part in ['left', 'middle', 'right']:
        ...
        save_to_db(uid, fname, part, sev, conf, cnt, ts)  # 記錄總數
